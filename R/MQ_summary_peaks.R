#'Number of peaks identified (read from MaxQuant summary.txt)
#'
#'@description Reads MaxQuant summary.txt output file and plots the number of
#'    peaks sequences identified Use the `add_groups` function first to add groups based
...
#'
#'@export
MQ_summary_peaks <- function(x, grouping = FALSE){
  if (grouping == TRUE) {
    p <- ggplot(x, aes(Experiment, Peaks , fill = my_groups))
  } else {
    p <- ggplot(x, aes(Experiment, Peaks))
  }
  p <- p + geom_bar(stat = "identity", position = "dodge")
  p <- p + theme_bw()
  p <- p + xlab("Sample")
  p <- p + ylab("Number of peaks")
  p <- p + ggtitle("Number of peaks per experiment.")
