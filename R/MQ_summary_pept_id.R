#'Number of peptide sequences identified (read from MaxQuant summary.txt)
#'
...
#'
#'@export
MQ_summary_pept_id <- function(x, grouping = FALSE){
  if (grouping == TRUE) {
    p <- ggplot(x, aes(Experiment, Peptide.Sequences.Identified , fill = my_groups))
  } else {
    p <- ggplot(x, aes(Experiment, Peptide.Sequences.Identified))
  }
  p <- p + geom_bar(stat = "identity", position = "dodge")
  p <- p + theme_bw()
  p <- p + xlab("Sample")
  p <- p + ylab("Number of peptides")
  p <- p + ggtitle("Number of peptide sequences identified per experiment.")
