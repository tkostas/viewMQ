#'Number of MS2 spectra identified (read from MaxQuant summary.txt)
#'
#'@description Reads MaxQuant summary.txt output file and plots the number of
#'    MS2 spectra identified Use the `add_groups` function first to add groups based
...
#'
#'@export
MQ_summary_MS2_id <- function(x, grouping = FALSE){
  if (grouping == TRUE) {
  ...
  p <- p + ggtitle("Number of MS2 spectra identified per experiment.")
