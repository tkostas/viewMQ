#'Number of MS1 spectra aquired (read from MaxQuant summary.txt)
#'
#'@description Reads MaxQuant summary.txt output file and plots the number of
#'    MS1 spectra aquired. Use the `add_groups` function first to add groups based
...
#'
#'@export
MQ_summary_MS1_spectra <- function(x, grouping = FALSE){
  if (grouping == TRUE) {
    p <- ggplot(x, aes(Experiment, MS , fill = my_groups))
  } else {
    p <- ggplot(x, aes(Experiment, MS))
  }
  p <- p + geom_bar(stat = "identity", position = "dodge")
  p <- p + theme_bw()
  p <- p + xlab("Sample")
  p <- p + ylab("Number of MS1 spectra")
  p <- p + ggtitle("Number of MS1 spectra aquired per experiment.")
