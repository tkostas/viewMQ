#'Number of isotopic patterns sequenced (z > 1) (read from MaxQuant summary.txt)
#'
...
#'
#'@export
MQ_summary_isopatterns_seq <- function(x, grouping = FALSE){
  if (grouping == TRUE) {
  ...
  p <- p + theme_bw()
  p <- p + xlab("Sample")
  p <- p + ylab("Number of isotopic patterns sequenced (z > 1)")
  p <- p + ggtitle("Number of isotopic patterns sequenced per experiment (z > 1)")
